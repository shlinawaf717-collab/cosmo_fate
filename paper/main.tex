...
points). A separately frozen public extension expands the fitted-truth
\lcdm{} null campaign to \NullFiveN{} mocks. The heat-death-compatible mass
remains consistent with $U(0,1)$ (KS $p=\NullFiveKSp$);
\NullFiveK/\NullFiveN{} mocks reach a Big Rip probability no larger than the
observed value, giving the finite-simulation result
$\hat p=\NullFivePlusOne$ (exact 95\% binomial interval
$[\NullFiveCPLow,\NullFiveCPHigh]$). Off-boundary power is truth-specific:
at the two registered $\wa=\pm0.60$ alternatives direction power is unity and
one-sided depth power is $0.90/0.98$, whereas at
$\wa=-0.15/+0.15$ they are $0.84/0.77$ and $0.33/0.18$. The six constrained
truth fits span $\Delta\chi^2=0$--$39.4$, so they are not equally supported
effect sizes. A two-parameter Wilks test gives $p=0.024$, but tests a
different statistic and is not an independent calibration of the posterior
...
classifier on a completed 500-mock fitted-truth \lcdm{} ensemble
(Sec.~\ref{sec:null}), measure its off-boundary power
(Sec.~\ref{sec:power}), and then measure the fate verdict's
...
real data push the Big Rip probability to $0.18\%$ under CPL. Two of
\NullFiveN{} fitted-truth null mocks are at least as deep, giving
$\hat p=\NullFivePlusOne$ and an exact 95\% binomial interval
$[\NullFiveCPLow,\NullFiveCPHigh]$. The Wilks
likelihood-ratio value \cite{Wilks1938} $p=0.024$ points in the same qualitative direction but
concerns a different null statistic. Registered off-boundary experiments give
unit direction power at $\wa=\pm0.60$ but only $0.84/0.77$ at
$\wa=-0.15/+0.15$, so power is not summarized by the outer points alone.
Third, in translation: four converged
...
The 500-mock and off-boundary-power results were not part of that original
local preregistration. They belong to a separately versioned PRD-strengthening
protocol, frozen after completion of the v1.x paper and before the WP2--WP7
extension results. The immutable historical checkpoint is tag
...
Its append-only \texttt{PRD-A} ledger records the WP2 execution-order
deviation and all later corrective scope changes.
...
\section{Off-boundary power}\label{sec:power}
...
\begin{table*}[t]
\caption{Registered WP3 truth-specific power. $\Delta\chi^2$ is relative to
...
\caption{WP3 direction and one-sided depth power at six registered constrained
...
compressed-CMB-conditioned. A separately frozen full-CMB/lensing CPL
extension is still in progress at this manuscript checkpoint and contributes
no scientific endpoint here. Direct late-time growth inference is separately
conditional on the WP6 data-availability and overlap gate; CMB lensing alone
must not be described as a complete direct-growth test. Growth data could
shift the window-interior signal. The four fitted grammars are not
...
pipeline-conditional finite-redshift preference: \NullFiveK{} of
\NullFiveN{} fitted-truth \lcdm{} mocks are at least as deep as the real CPL
tail, giving the registered plus-one value \NullFivePlusOne, while a distinct
Wilks test gives $p=0.024$. Off the boundary, the direction classifier is
powerful under the two registered outer-point alternatives, but the weaker
inner-point and depth results forbid a blanket power claim. Translating the
finite-redshift preference into fate remains conditional. A quintessence
prior excludes the rip before data; four
...
WP2 extension replayed the same generator stream and appended indices
...
trace. The extension additionally versions the WP2 completion and endpoint
artifacts and the WP3 truth fits, 606-case completion audit, exact power table,
and profile-$\chi^2$-annotated figure. All
