...
if __name__ == '__main__':
    ...
    for tracer, namespaces in tracers.items():
        for observable in ['shapefit'] + (['shapefit_bao'] if tracer != 'all' else []):
            ...
            if observable == 'shapefit' and tracer == 'all':  # remove Lya
            ...
            config_tracer['observable_name'] = {'shapefit': 'spectrum-poles-rotated', 'shapefit_bao': 'spectrum-poles-rotated+bao-recon'}[observable]
            dump(config_tracer, basename + '.yaml')
            py_template = '''from .desi_shapefit_bao_all import *


class {name}(desi_shapefit_bao_all):
    r"""
    DESI ShapeFit (optionally with post-recon BAO) likelihood for {tracer}.
    """
            '''
