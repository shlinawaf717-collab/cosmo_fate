...
## E-2026-07-21（学术定位与预注册溯源批次）

- **E-21（预注册身份）** 保留 `preregistered`，并限定为“本地版本控制预注册”。
  `prereg-v1`/`98abc20` 记录于 10:26，首个终局结果 `76ed25d` 记录于 11:29，且
  前者是后者的祖先。公共仓库建立较晚，正文明确不把它冒充第三方平台注册；新增
  `PREREGISTRATION_PROVENANCE.md` 固化提交、树、文件 SHA-256 与限制。
- **E-22（证据边界）** 摘要、导言、讨论和结论区分方向、深度与命运翻译：D0 的
  fitted-truth mock 深度只支持 CPL+P1+压缩 CMB 管线内的条件性偏好；D1--D4
  方向稳定但未单独 null 校准；跨语法数值是原生先验下的敏感性诊断，不是共同先验比较。
- **E-23（文献与物理措辞）** 增补 Li et al. (2012) 的宇宙终局文献；删除 Dyson
  可用能旁论；将 DS/DECAY 的合并限定为“无限膨胀且无有限时奇点”的操作性宏类，
  不再暗示二者热力学等价。零链重跑、零分类器变更、零核心数值变更。
- **E-24（逐语法 P1 溯源）** 新增 A-007，说明 CPL、JBP/BA、BIN4 的早期物质
  主导条件分别为 `w0+wa<0`、`w0<0`、`w4<0`。该翻译与首批 F_param 输出首次
  同在 `777816f` 归档，现有历史不能区分提交内部先后，故只作事后实现澄清，不
  追溯性地称为冻结文本中已显式预注册的细节。A-006 的更强 BIN4 门槛保持生效。
- **E-25（A-003 作废标识）** 在论文 A-003 开头直接标注其公式已由 A-005 取代，
  仅为历史溯源保留，当前结果未使用。
- **E-26（拟合比较措辞）** 将“comparably fitting”收紧为“similar archived
  chain-minimum AIC values”，明确没有专门极值优化或跨语法证据计算。
- **E-27（预注册标题与范围）** 标题改为 “A locally preregistered fragility
  audit”；正文明确这是既有公开数据上的终局翻译审计预注册，不声称早于数据接触，
  也不声称具有第三方独立时间戳。
- **E-28（正式文献元数据）** 补齐 Union3、Wang--Mota、DES-Dovekie、compressed
  evidence、Wolf--Read 的正式期刊卷期、文章号、年份和 DOI。零分析变动。
