...
The draft uses several standard cosmological reference points only as
context: CPL-like dark-energy parametrizations
...
posterior class masses depending on the noisy posterior center. Phase 2
selected three examples from the same repeated-experiment ensemble:
...
future-behavior diagnostics and then returns a discrete label. The
method paper represents this by four margins:
...
Boundary calibration requires an explicit null distribution. For a
...
The Phase 5 case study fills Direction, Depth, and Boundary. It does not
fill Evidence and Horizon. That incompleteness is intentional. A method
paper should not silently promote an available class-mass result into an
unavailable model preference or extrapolation-horizon result.
...
Evidence & Not evaluated by the Phase 5 case-study figure; a science
paper must add likelihood-ratio, AIC/BIC, or Bayes-factor evidence
separately. \\
Boundary & max OTHER=0.00142; max boundary fraction=0.00087; mock000
P\_heat=0.52275. \\
Horizon & Not evaluated by the Phase 5 case-study figure; a science
paper must report constraint horizon or explicitly mark it
unavailable. \\
\end{longtable}
}
...
therefore asks which perturbations preserve the boundary-null behavior
and which correctly fail diagnostics.
...
experiments. & Phase 1 mean=0.50096, variance=0.08356, KS p=0.16459;
Phase 2 KS p=0.05819. \\
supported & A 50 percent direction rate is not automatically a failed
classifier near the boundary. & Phase 5 LCDM mocks have
P\_heat\textgreater0.5 in 0.50 of mocks while the P\_heat null shape
passes KS p=0.25266. \\
...
Boundary, and Horizon. & Phase 6 defines all five layers and marks
Evidence and Horizon as unfilled in the case study. \\
not claimed & The method establishes the real universe's final fate. &
Excluded by Phase 0 non-goals and by the Phase 5 case-study scope. \\
not claimed & The case study proves dynamical dark energy or model
preference. & Evidence layer is explicitly unavailable in Phase 6 and
must be supplied by a separate science analysis. \\
not claimed & Curved or miscalibrated boundaries always preserve
uniformity. & Phase 7 shows off-boundary truth and posterior-width
mismatch fail; beta=1.5 curvature is labeled a diagnostic. \\
\end{longtable}
}
...
Reference keys are inherited from \texttt{paper/refs.bib} and should be
rendered from that file in a submission build:
...
\item
  \citep{DESIDR2}
\item
  \citep{ChenHuangWang2018}
\item
  \citep{ChevallierPolarski2001}
\item
  \citep{Linder2003}
\item
  \citep{CAMB}
...
\item
  \citep{Dyson1979}
\item
  \citep{KraussTurner1999}
\item
  \citep{KraussStarkman2000}
\item
  \citep{Caldwell2003}
\end{itemize}

\bibliography{/Users/nocide/cosmo_fate/paper/method/submission/refs.bib}
...
\begin{figure}[p]
\centering
\includegraphics[width=0.95\textwidth]{figures/phase1_toy_boundary_uniformity.png}
\caption{Figure 1: One-dimensional boundary-null calibration.}
\end{figure}
\begin{figure}[p]
\centering
\includegraphics[width=0.95\textwidth]{figures/phase2_w0wa_boundary.png}
\caption{Figure 2: Two-dimensional w0-wa boundary analogue.}
\end{figure}
\begin{figure}[p]
\centering
\includegraphics[width=0.95\textwidth]{figures/phase3_classifier_framework.png}
\caption{Figure 3: Abstract fate-classifier framework.}
\end{figure}
\begin{figure}[p]
\centering
\includegraphics[width=0.95\textwidth]{figures/phase4_null_calibration.png}
\caption{Figure 4: Finite-mock null-calibration protocol.}
\end{figure}
\begin{figure}[p]
\centering
\includegraphics[width=0.95\textwidth]{figures/phase5_cosmology_mock_case.png}
\caption{Figure 5: Real-pipeline LCDM mock case study.}
\end{figure}
\begin{figure}[p]
\centering
\includegraphics[width=0.95\textwidth]{figures/phase6_reporting_standard.png}
\caption{Figure 6: Five-layer reporting standard.}
\end{figure}
\begin{figure}[p]
\centering
\includegraphics[width=0.95\textwidth]{figures/phase7_robustness.png}
\caption{Figure A1: Robustness diagnostics.}
\end{figure}
