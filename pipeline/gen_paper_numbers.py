...
null500 = load("runs/prd_extension/null500/endpoints.json")["endpoints"]
wp3_power = load("runs/prd_extension/wp3_power/power_report.json")
...
def power_row(truth_id):
    return next(row for row in wp3_power["rows"] if row["truth_id"] == truth_id)


def rate_interval(row, rate_key, interval_key):
...
macros = [
    ...
    # Prospectively frozen WP2 500-mock extension (Sec. VI)
    ("NullFiveN", f"{null500['primary_depth']['N']}"),
    ("NullFiveK", f"{null500['primary_depth']['K']}"),
    ("NullFiveTailPct", f"{null500['primary_depth']['fraction'] * 100:.2f}"),
    (
        "NullFivePlusOne",
        f"{null500['finite_simulation_lower_tail']['p_plus_one']:.5f}",
    ),
    (
        "NullFiveCPLow",
        f"{null500['tail_probability_interval']['interval'][0]:.6f}",
    ),
    (
        "NullFiveCPHigh",
        f"{null500['tail_probability_interval']['interval'][1]:.4f}",
    ),
    ("NullFiveDirectionK", f"{null500['direction']['K']}"),
    ("NullFiveDirectionPct", f"{null500['direction']['fraction'] * 100:.1f}"),
    (
        "NullFiveDirectionLowPct",
        f"{null500['direction']['exact_binomial_95_interval'][0] * 100:.1f}",
    ),
    (
        "NullFiveDirectionHighPct",
        f"{null500['direction']['exact_binomial_95_interval'][1] * 100:.1f}",
    ),
    ("NullFiveKSp", f"{null500['uniformity_diagnostic']['p_value']:.3f}"),
    (
        "NullFiveRipMinPct",
        f"{null500['primary_depth']['minimum_null_P_RIP'] * 100:.3f}",
    ),
    (
        "NullFiveOtherMaxPct",
        f"{null500['boundary_budget']['OTHER']['maximum'] * 100:.3f}",
    ),
    (
        "NullFiveBoundaryMaxPct",
        f"{null500['boundary_budget']['boundary_fraction']['maximum'] * 100:.3f}",
    ),
...
for macro_stem, truth_id in (
    ...
    row = power_row(truth_id)
    macros.extend(
        [
            (
                f"{macro_stem}DeltaChi",
                f"{row['delta_chi2_from_best_registered_truth']:.2f}",
            ),
            (
                f"{macro_stem}Direction",
                rate_interval(
                    row,
                    "direction_power",
            ...
            (
                f"{macro_stem}Depth",
                rate_interval(row, "depth_power", "depth_power_exact95"),
            ),
            (
                f"{macro_stem}False",
                rate_interval(
